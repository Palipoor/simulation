...
class Simulator:
	...
	def check_finish_criteria(self):
		return self.get_accuracy() >= self.MIN_ACCURACY or self.total_jobs_started == self.JOB_COUNT
	...
	def get_accuracy(self):
		...
		for k in self.reports[-1].keys():
			error = abs(self.reports[-1][k] - self.reports[-2][k]) / self.reports[-1][k] if self.reports[-1][k] else 0
			min_acc = min(min_acc, 1 - error)
